...
# usage example: bash run_bsub.sh <input_dir> <output_dir> QUEUE="new-all.q"
...
for file in `ls -1 $input_dir`; do
    ...
    temp_sam_path=${output_dir}/${sample_name}_temp.sam
    temp_bam_path=${output_dir}/${sample_name}_temp.bam
    output_bam_path=${output_dir}/${sample_name}.bam

    # @@@-- write header ------------------------------------------------- @@@
    command_write_header="samtools view -H ${input_sam_path} >> ${temp_sam_path}"
    ...
    command_filter="samtools view -F 4 -f 0x2 ${input_sam_path} | awk -F '\t' '{if (\$9> ${SIZE_TO_FILTER_BY}) print \$0}' >> ${temp_sam_path}" 
    ...
    # @@@-- convert to bam ----------------------------------------------- @@@
    command_convert="samtools view -b ${temp_sam_path} >> ${temp_bam_path}"
